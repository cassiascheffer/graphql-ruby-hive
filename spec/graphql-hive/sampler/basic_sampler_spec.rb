...
RSpec.describe GraphQLHive::Sampling::BasicSampler do
  ...
  describe "#sample?" do
    ...
    let(:duration) { 100 }
    let(:operation) { GraphQLHive::Tracing::Operation[timestamp, queries, results, duration] }
    ...
    context "with at least once sampling" do
      ...
      context "when provided a custom key generator" do
        ...
        it "tracks operations by their custom keys" do
          ...
          different_operation = GraphQLHive::Tracing::Operation[timestamp, queries, results, duration]
